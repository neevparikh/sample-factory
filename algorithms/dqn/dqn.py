...
Methods of the DQN class initiate all other components (rollout & policy workers and learners) in the main thread,
and then fork their separate processes.
All data structures that are shared between processes are also created during the construction of DQN.
...
from algorithms.base.algorithm import ReinforcementLearningAlgorithm
from algorithms.dqn.actor_worker import ActorWorker
from algorithms.utils.algo_utils import make_env_func, iterate_recursively, set_global_cuda_envvars
from algorithms.dqn.learner import LearnerWorker
from algorithms.dqn.policy_worker import PolicyWorker
# from algorithms.appo.population_based_training import PopulationBasedTraining
from algorithms.dqn.shared_buffers import SharedBuffers
...
class DQN(ReinforcementLearningAlgorithm):
    ...
    def add_cli_args(cls, parser):
        ...
        p.add_argument('--dqn_epochs', default=1, type=int, help='Number of training epochs before a new batch of experience is collected')
